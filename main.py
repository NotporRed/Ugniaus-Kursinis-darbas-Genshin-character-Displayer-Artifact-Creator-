...
class Game:
    def __init__(self):
        ...
        self.selection_weapon = Selection_weapon(
        ...
        self.states = {
            ...
            'selection_weapon': self.selection_weapon}
...
class Selection():
    ...
    def fillter_the_list(self):
        self.filterred_list = [
            x for x in self.character_list if x.startswith(
                self.user_text)]
    ...
    def create_button(self):
        self.button.append(Button(self.display, (104, 71, 104),
                           415, 500, 450, 150, self.filterred_list[self.index]))
        self.button.append(
            Button(
                self.display,
                (104,71,104),
    ...
    def run(self, event_list):
        ...
        png = pygame.image.load(
            "Icons/" + self.filterred_list[self.index].replace(" ", "_") + "_Icon.png")
        ...
        for event in event_list:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_BACKSPACE:
                    ...
                    if event.key in range(
                            pygame.K_a,
                            pygame.K_z +
                            1) or event.key == pygame.K_SPACE:
...
class Selection_weapon():
    ...
    def fillter_the_list(self):
        self.filterred_list = [
            x for x in self.weapon_list if x.startswith(
                self.user_text)]
        if not len(self.filterred_list):
...
class Character_screen():
    ...
    def print_text(self,atr,hp=0,atk=0,defence=0,cr=0,cd=0,em=0,er=0,healing_bonus=0,phys=0,dmg=0):
        lv_mult = float(
            self.scaling.level_multiplier(
                self.level,
                self.character.give_item(3)))
        ...
        Text(self.display, "Level: " +
             str(self.level) +
             "/90", ('white'), 40, 80, 35)
        Text(self.display, "A" + str(self.asc), ('white'), 40, 130, 20)

        text_hp = self.character.give_item(5) * lv_mult + lv_asc * self.scaling.max_asc_value(
            self.character.give_item(1), hp=1) + hp + (
            self.weapon.give_value() * 100 if self.weapon.give_sub() == "HP%" else 0)
        text_atk = self.character.give_item(6) * lv_mult + lv_asc * self.scaling.max_asc_value(
            self.character.give_item(1), atk=1) + atk + self.weapon.give_atk() + (
            self.weapon.give_value() * 100 if self.weapon.give_sub() == "ATK%" else 0)
        text_defence = self.character.give_item(7) * lv_mult + lv_asc * self.scaling.max_asc_value(
            self.character.give_item(1), defence=1) + defence + (
            self.weapon.give_value() * 100 if self.weapon.give_sub() == "DEF%" else 0)
        text_crit_rate = (0.05 + cr) * 100 + (self.weapon.give_value()
                                              * 100 if self.weapon.give_sub() == "CR" else 0)
        text_crit_dmg = (0.5 + cd) * 100 + (self.weapon.give_value()
                                            * 100 if self.weapon.give_sub() == "CD" else 0)
        text_elemental_mastery = 0 + em + \
            (self.weapon.give_value() if self.weapon.give_sub() == "EM" else 0)
        text_energy_recharge = (1 + er) * 100 + (self.weapon.give_value()
                                                 * 100 if self.weapon.give_sub() == "ER" else 0)
            ...
            for x in range(len(all_artifacts)):
                for key in all_artifacts[x].keys():
                    match key:
                        ...
                        case "ER":
                            text_energy_recharge += (
                                all_artifacts[x].get("ER"))

                        case "EM":
                            text_elemental_mastery += all_artifacts[x].get(
                                "EM")
        ...
        Text(self.display, "Def: " +
             str(round(text_defence)), ('white'), 40, 230, 25)

        Text(self.display, "Crit rate: " +
             str(round(text_crit_rate, 1)) +
             "%", ('white'), 40, 270, 25)
        Text(self.display, "Crit dmg: " +
             str(round(text_crit_dmg, 1)) +
             "%", ('white'), 40, 300, 25)

        Text(self.display, "Elemental mastery: " +
             str(round(text_elemental_mastery)), ('white'), 40, 340, 25)
        Text(self.display, "Energy recharge: " +
             str(round(text_energy_recharge, 1)) +
             "%", ('white'), 40, 370, 25)
        Text(self.display, "Healing bonus: " +
             str(round(text_healing_bonus, 1)) +
             "%", ('white'), 40, 400, 25)

        if phys != 0:
            Text(str(self.display, self.character.give_item(2)) + "DMG %: +" +
                 str(round((dmg + atrifact_dmg) * 100, 1)), ('white'), 40, 430, 25)
        else:
            Text(
                self.display,
                self.character.give_item(2) +
                ": +" +
                str(
                    round(
                        (dmg) *
                        100 +
                        self.weapon.give_value *
                        100 if self.weapon.give_sub == "Phys DMG%" else 0,
                        1)),
                ('white'),
                40,
                430,
        ...
        if em == 0:
            Text(self.display, "Ascenion Stat: +" +
                 str(round(hp +
                           atk +
                           defence +
                           cr +
            ...
            Text(self.display, "Ascenion Stat: " +
                 str(round(em)) + " " + atr, ('white'), 50, 450, 25)
    ...
    def run(self, event_list):
        ...
        atr_mult = self.scaling.atribute_position(
            self.character.give_item(4), self.asc, self.character.give_item(3))
...
class Button:
    ...
    def is_hovered(self):
        return pygame.MOUSEBUTTONUP and self.rect.collidepoint(
...
class Artifact:
    ...
    def generate_subs(self):
        ...
        for n in range(4):
            x = self.random_atr()
            if x and x[0] in self.subs.keys():
                self.subs[x[0]] += x[1]
            else:
                if x:
                    self.subs[x[0]] = x[1]
        ...
        if not check:
            ...
            for row in ws.iter_rows(min_row=2, max_row=86, max_col=7):
                ...
                if len(base_stats_data) == 7:
                    self.character_catalogue.append(
                        Character_stats(*base_stats_data))
                    base_stats_data = []
    ...
    def give_item_list(self):
        ...
        for x in range(85):
            self.character_list.append(
                self.character_catalogue[x].give_item(1))
